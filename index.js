...
function parseFixtures(html) {
  ...
  $('table.table-main tr').each((_, tr) => {
    ...
    if (aTexts.length >= 2) {
      fixtures.push({ home: aTexts[0].replace(/\s+FC$/i, '').replace(/\s+CF$/i, ''), away: aTexts[1].replace(/\s+FC$/i, '').replace(/\s+CF$/i, '') });
    }
  });
...
function buildMetricsForFixture(home, away, stats) {
  ...
  const htWDL     = `${spacedWDL(homeH)} - ${spacedWDL(awayA)}`;
  const ftWDL     = `${spacedWDL(totalH)} - ${spacedWDL(totalA)}`;

  return { match: `${hKey} vs ${aKey}`, goal_ratio: goalRatio, wdl_ratio: wdlRatio, ha_record: haRecord, ht_wdl: htWDL, ft_wdl: ftWDL };
}
...
app.get('/api/fixtures', async (req, res) => {
  ...
  } catch(err) { console.error(err); res.status(500).json({ error:'Scrape failed', details: err.message }); }
});

// Health check
app.get('/', (req,res) => res.send('RSBS BetExplorer API running. Use /api/fixtures'));
